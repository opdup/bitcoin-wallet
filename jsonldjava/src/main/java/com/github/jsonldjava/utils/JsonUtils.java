...
public class JsonUtils {
    ...
    /**
     * The user agent used by the default {@link OkHttpClient}.
     *
     * This will not be used if is called with
     * a custom client.
     */
    ...
    private static volatile OkHttpClient DEFAULT_HTTP_CLIENT;
     ...
     * resolved from the JsonLdUrle, to an object that can b used as input for
     * the {@link JsonLdApi} and {@link JsonLdProcessor} methods.
     *
     * @param url
     *            The JsonLdUrl to resolve
     * @param httpClient
     *            The {@link OkHttpClient} to use to resolve the URL.
     * @return A JSON Object.
     * @throws JsonParseException
     *             If there was a JSON related error during parsing.
     * @throws IOException
     *             If there was an IO error during parsing.
     */
    public static Object fromURL(java.net.URL url, OkHttpClient httpClient)
    ...
    public static OkHttpClient getDefaultHttpClient() {
        OkHttpClient result = DEFAULT_HTTP_CLIENT;
        if (result == null) {
    ...
    public static OkHttpClient createDefaultHttpClient() {
        //final OkHttpClient.Builder result = createDefaultHttpClient(cacheConfig);

        //final OkHttpClient.Builder result = new OkHttpClient().cache(new Cache(c));

        Context context = null;
        int cacheSize = 10 * 1024 * 256;
        OkHttpClient.Builder builder = new OkHttpClient.Builder().cache(new Cache(context.getCacheDir(), cacheSize));
        OkHttpClient client = builder.build();
        return client;
    }

    /*public static CacheControl createDefaultCacheConfig() {
        return new CacheControl().

        return Cache.custom().setMaxCacheEntries(500).setMaxObjectSize(1024 * 256)
                .setSharedCache(false).setHeuristicCachingEnabled(true)
                .setHeuristicDefaultLifetime(86400).build();

    }*/

    /*public static OkHttpClient createDefaultHttpClient(final CacheConfig cacheConfig) {
